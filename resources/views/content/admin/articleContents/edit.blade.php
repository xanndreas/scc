...
@section('title', trans('global.edit') . " " . trans('cruds.articleContent.title'))
...
@section('content')
    <!-- Modern -->
    <div class="col-12">
        <h5>{{ trans('global.edit') }} {{ trans('cruds.articleContent.title_singular') }}</h5>
    </div>

    <div class="card">
        <div class="card-body">
            <form method="POST" action="{{ route("admin.article-contents.update", [$articleContent->id]) }}" enctype="multipart/form-data">
                @method('PUT')
                @csrf
                ...
                <div class="row g-3">
                    <div class="col-sm-12">
                        ...
                        <input class="form-control {{ $errors->has('title') ? 'is-invalid' : '' }}" type="text" name="title" id="title" value="{{ old('title', $articleContent->title) }}" required>
                        @if($errors->has('title'))
                    ...
                    <div class="col-sm-12">
                        ...
                        <textarea class="form-control ckeditor {{ $errors->has('page_text') ? 'is-invalid' : '' }}" name="page_text" id="page_text">{!! old('page_text', $articleContent->page_text) !!}</textarea>
                    ...
                    <div class="col-sm-12">
                        ...
                        <select class="form-control select2 {{ $errors->has('categories') ? 'is-invalid' : '' }}" name="categories[]" id="categories" multiple>
                            @foreach($categories as $id => $category)
                                <option value="{{ $id }}" {{ (in_array($id, old('categories', [])) || $articleContent->categories->contains($id)) ? 'selected' : '' }}>{{ $category }}</option>
                            @endforeach
                        </select>
