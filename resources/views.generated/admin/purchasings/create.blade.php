...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.purchasing.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.purchasings.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('purchasing_details') ? 'is-invalid' : '' }}" name="purchasing_details[]" id="purchasing_details" multiple required>
                    @foreach($purchasing_details as $id => $purchasing_detail)
                        <option value="{{ $id }}" {{ in_array($id, old('purchasing_details', [])) ? 'selected' : '' }}>{{ $purchasing_detail }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('purchasing_transaction_number') ? 'is-invalid' : '' }}" type="text" name="purchasing_transaction_number" id="purchasing_transaction_number" value="{{ old('purchasing_transaction_number', '') }}" required>
