...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.selling.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.sellings.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('selling_details') ? 'is-invalid' : '' }}" name="selling_details[]" id="selling_details" multiple required>
                    @foreach($selling_details as $id => $selling_detail)
                        <option value="{{ $id }}" {{ in_array($id, old('selling_details', [])) ? 'selected' : '' }}>{{ $selling_detail }}</option>
                    @endforeach
                </select>
