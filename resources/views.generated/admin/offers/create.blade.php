    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.offers.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('offer_details') ? 'is-invalid' : '' }}" name="offer_details[]" id="offer_details" multiple required>
                    @foreach($offer_details as $id => $offer_detail)
                        <option value="{{ $id }}" {{ in_array($id, old('offer_details', [])) ? 'selected' : '' }}>{{ $offer_detail }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('offering_expired_date') ? 'is-invalid' : '' }}" type="text" name="offering_expired_date" id="offering_expired_date" value="{{ old('offering_expired_date') }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('offering_number') ? 'is-invalid' : '' }}" type="text" name="offering_number" id="offering_number" value="{{ old('offering_number', '') }}" required>
